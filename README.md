...
**注意**：只需要修改 JSON 文件，不需要创建任何 JS 文件！插件会自动生成所有页面。
...
1. 在 `products/` 目录创建新的 JSON 文件（如：`new-product.json`）
2. 按照上述数据格式填写产品信息
3. 将产品图片放在 `static/img/products/` 目录
4. 重新构建网站

**优势**：不需要创建任何 JS 文件！插件会自动生成：
- 产品列表页面（自动更新）
- 产品详情页面（自动创建）
- 所有链接和导航（自动生成）
...
| 操作 | 传统方案 | 我们的方案 |
|------|----------|------------|
| **添加产品** | 创建 JS 文件 + JSON 数据 | 只需创建 JSON 文件 |
| **修改产品** | 修改 JS 文件 + JSON 数据 | 只需修改 JSON 文件 |
| **删除产品** | 删除 JS 文件 + JSON 数据 | 只需删除 JSON 文件 |
| **维护成本** | 高（需要管理 JS 文件） | 低（只需管理 JSON 文件） |
| **扩展性** | 差（上百个 JS 文件难管理） | 优（JSON 文件易于批量处理） |
...
1. **创建 JSON 文件**：
   ```bash
   # 在 products/ 目录创建 new-product.json
   ```
